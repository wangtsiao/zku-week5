...
template ProcessTx(k){
    ...
    hasher.inputs[0] <== receiver_pubkey[0];
    hasher.inputs[1] <== receiver_pubkey[1];
    hasher.inputs[2] <== receiver_balance + amount;
    updated_tree.leaf[0] <== receiver_pubkey[0];
    updated_tree.leaf[1] <== receiver_pubkey[1];
    updated_tree.leaf[2] <== receiver_balance;
    for (var i=0; i<k; i++) {
        updated_tree.pathElements[i] <== hasher.out;
        updated_tree.pathIndices[i] <== 0;
    }
